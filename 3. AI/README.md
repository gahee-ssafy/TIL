...
![1020-1](./1020-1.png)
# 텍스트 파운데이션 모델(LLM)
- (특별) 규모가 커질수록 성능이 체계적으로 오르는 규모의 법칙(Scaling law)와, 일정 규모를 넘어서 나타나는 **창발성(예: in-context learning)**이 관찰됨.
    - 창발성 : 모델 크기·데이터·학습량을 일정 임계치 이상 키우면, 이전엔 거의 못 하던 능력이 갑자기 나타나는 현상.
- 정의: 초거대 트랜스포머를 자기지도→정렬로 다듬어 범용 언어 능력을 갖춘 모델.
- 추론: 분포 위에서 디코딩(Greedy/Beam/Top-k/Top-p, Temperature)과 **프롬프트 설계(CoT)**가 품질을 좌우.
- 응용/한계: 멀티모달·업무특화로 확장되지만 환각·탈옥·평가편향을 관리해야 함.
  

# AI 강의 7차 
![1021-1](./1021-1.png)
![1021-2](./1021-2.png)
1. Pre-training vs Post-training ; 사전학습은 언어, 패턴을 배우는 단계, 사후학습은 대화단계 
   1. 사전학습은 다음 단어를 예측하는 데 강점을 가지지만, 질문에 대한 대답을 못함. 즉 대화 불가 
   2. 사후학습은 의도를 파악해 질문에 대한 대답을 한다. ->  Instruction-tuning, RLHF 
2. Instruction이 뭐냐? 사람의 지시를 따르도록 학습하는 단계 
   1. step.1 : 사전학습 + step.2 : 파인튜닝 
      1. 유저의 의도를 제대로 반영하지 못함. -> 파인튜닝으로 추가학습한다.  
   2. 한계: 비용 + 창의성을 요구하는 태스크는 정답이 없다. -> RLHF
![1021-3](./1021-3.png)
![1021-4](./1021-4.png)
![1021-5](./1021-5.png)
3. Reinforcemen Learning from Human Feedback (RLHF)
   1. 인간의 선호를 만족시키자! 인간선호도 중심 대답 (Human Preference)
   2. ![1021-6](./1021-6.png)
   3. ![1021-7](./1021-7.png)
4. RLHF가 성능의 종착지라면, 선호의 일관성 문제는 해결할 수 있을까? 예를 들어, 리워드 해킹이 있다.
   1. 리워드 해킹은 강화학습에서 자주 발생하는 문제로, 챗봇들이 정답의 여부와는 관계없이 '도움이 되어 보이는 정답'을 생성하고 이로인한 '환각' 문제를 발생시킨다.  
   2. ![1021-8](./1021-8.png) 
   3. 정답이 분명한 문제들만 리워드
