...
from .forms import CustomUserCreationForm, CustiomUserChangeForm
...
def update(request):
    if request.method == 'POST':
        form = CustiomUserChangeForm(request.POST, instance=request.user)
        if form.is_valid():
            form.save()
            return redirect('articles:index')
    else:
        form = CustiomUserChangeForm(instance=request.user)
